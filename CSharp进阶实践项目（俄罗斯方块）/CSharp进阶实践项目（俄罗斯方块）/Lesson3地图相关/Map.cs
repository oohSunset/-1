...
namespace CSharp进阶实践项目_俄罗斯方块_
{
    class Map : IDraw
    {
        ...
        public Map(GameScene scene)
        {
            ...
            //为了方便外部得到地图的高的边界 直接在这里记录 避免修改代码时多次修改
            h = Game.h - 6;
        ...
        public void CheckClear()
        {
            ...
            {
                //必须满足条件 才证明满了
                //小方块计数 == w （这个w已经是去掉了左右两边固定墙壁的数量）
                if (recordInfo[i] == w)
                {
                    //1.这一行的所有小方块移除
                    for (int j = 0; j < dynamicWalls.Count; j++)
                    {
                        //当前通过动态方块的 y 计算它在哪一行 如果行号
                        //和当前记录索引一致 就证明 应该移除
